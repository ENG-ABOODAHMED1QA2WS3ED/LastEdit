...
class NameNormalizer {
  ...
  static _scoreOnePair(bankName, systemName) {
    ...
    // Common name filter - these names match too easily
    const commonNames = new Set([
      ...
      "ايمان", "مريم", "اسماء", "محمد", "mohammed", "mohammad", "ahmed",
      "mahmoud", "ali", "ibrahim", "khalid", "omar", "yousef", "hassan",
      "hussein", "said", "mustafa", "sami", "rami", "hani", "walid",
    ]);

    // Count how many matched tokens are "distinctive" (not common)
    const distinctiveMatched = matchedTokens.filter(t => !commonNames.has(t)).length;
    const distinctiveSystem = systemTokensAr.filter(t => !commonNames.has(t)).length;
    ...
    if (totalSystem >= 2 && matchedCount >= totalSystem - 1 && (matchedCount >= 2 || distinctiveMatched >= 1)) {
      return { score: distinctiveMatched >= 1 ? 85 : 55, method: 'near_subset', matched_candidate: bankName };
    }
    ...
    if (totalSystem >= 2 && phoneticMatches >= totalSystem - 1) {
      return { score: 45, method: 'near_phonetic', matched_candidate: bankName };
    }
  ...
  static _tokensMatch(token1, token2) {
    ...
    // For short tokens (<=4 chars), require exact match only
    // This prevents "جمال" matching "جميل" (dist=1 but different names)
    ...
    // Strict: only allow dist=1 for tokens with 5+ chars
    if (longer >= 5 && dist <= 1) return true;
    // Very long tokens: allow dist=2
